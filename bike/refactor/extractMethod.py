...
from bike.parsing import ParserError
from bike.parsing.fastparserast import Class, ElementHasNoParentException
...
class ExtractMethod(object):
    ...
    def insertNewFunctionIntoSrcLines(self,srclines,newfn,insertpos):
        ...
        tabwidth = self.getTabwidthOfFunctionBody()
    ...
    def deduceIfIsMethodOrFunction(self):
        self.isAMethod = 0
        try: parent = self.fn.getParent()
        except ElementHasNoParentException: # must be just code in an unpackaged module
            return
...
def getFunctionArgs(lines):
    ...
    try:
        ...
        raise ParserException("couldnt parse:"+doctoredline)
    return compiler.walk(ast, FunctionVisitor()).result
