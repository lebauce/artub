...
class helpers:
    ...
    def findReferencesIncludingDefn_helper(self,src,lineno,colno):
        writeTmpTestFile(src)
...
class TestFindReferences(BRMTestCase,helpers):
    def test_findsSimpleReferencesGivenAssignment(self):
        ...
        def foo():
            ...
            print a
        """)
        refs = self.findReferences_helper(src,3,10)
        self.assertEqual(tmpfile,refs[0].filename)
        self.assertEqual(3,refs[0].lineno)
        self.assertEqual(10,refs[0].colno)
        self.assertEqual(100,refs[0].confidence)
    ...
    def test_findsSimpleReferencesGivenReference(self):
        ...
        refs = self.findReferencesIncludingDefn_helper(src,3,10)
    ...
    def test_findsReferencesToOtherAssignments(self):
        ...
        def foo():
            ...
            a = 4
        """)
        refs = self.findReferences_helper(src,2,4)
        assert refs[0].filename == tmpfile
    ...
    def test_findsFunctionArg(self):
        ...
        def foo(a):
            print a
        """)
        refs = self.findReferencesIncludingDefn_helper(src,2,10)
        assert refs[0].filename == tmpfile
    ...
    def test_findsFunctionArgWithDefault(self):
        ...
        def foo(a=None, b=None):
            print a, b
        """)
        refs = self.findReferences_helper(src,1,4)
    ...
    def test_findsFunctionArgWithDefault2(self):
        ...
        def foo(a=None, b=None):
            print a, b
        """)
        refs = self.findReferencesIncludingDefn_helper(src,2,13)
        assert refs[0].filename == tmpfile
        assert refs[0].lineno == 1
    ...
    def test_findsFunctionArgWithDefault3(self):
        src=trimLines("""
        def foo(a='hello', b=None):
            print a, b
        """)
        refs = self.findReferencesIncludingDefn_helper(src,2,13)
        assert refs[0].filename == tmpfile
        assert refs[0].lineno == 1
        assert refs[0].colno == 19
        assert refs[0].confidence == 100
    ...
    def test_findsReferencesGivenFunctionArg(self):
        ...
        def foo(a):
            print a
        """)
        refs = self.findReferences_helper(src,1,8)
        assert refs[0].filename == tmpfile
        assert refs[0].lineno == 2
    ...
    def test_findsRefToSelfAttribute(self):
        ...
        class MyClass:
           ...
           def myMethod(self):
               print self.a
        """)
        refs = self.findReferences_helper(src,3,12)
        assert refs[0].filename == tmpfile
    ...
    def test_findsMethodDecl(self):
        ...
        class TheClass:
            def theMethod(self):
                pass
        """)
        refs = self.findReferencesIncludingDefn_helper(src,2,8)
        self.assertEqual(refs[0].filename,tmpfile)
        self.assertEqual(refs[0].lineno,2)
        self.assertEqual(refs[0].colno,8)
        self.assertEqual(refs[0].confidence,100)

    def test_findsRefToKeywordParamWhenArgIsSameName(self):
        src = trimLines("""
        def foo(a=None):
            bar(a=a)
        """)
        refs = self.findReferencesIncludingDefn_helper(src,2,10)
        self.assertEqual(tmpfile,refs[1].filename)
        self.assertEqual(2,refs[1].lineno)
        self.assertEqual(10,refs[1].colno)
        self.assertEqual(100,refs[1].confidence)

    def test_findsRefToKeywordParamWhenArgIsDifferentName(self):
        src = trimLines("""
        def foo(a=None):
            bar(b=a)
        """)
...
class TestFindReferencesUsingFiles(BRMTestCase):
    ...
    def test_findsPossibleMatchesButNotWith100PercentConfidence(self):
        src=trimLines("""
        class MyClass:
            def theMethod(self):
                pass
        ...
        a.theMethod()
        """)
        refs = self.helper("pass",src,2,8)
        assert refs[0].filename == pkgstructureFile2
        assert refs[0].lineno == 5
        assert refs[0].colno == 2
        assert refs[0].confidence == 50
