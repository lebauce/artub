...
class SocketIO(object):
    ...
    def __init__(self, sock, objtable=None, debugging=False):
    ...
    def _getresponse(self, myseq, wait):
        self.debug("_getresponse:myseq:", myseq, threading.currentThread() is self.sockthread)
    ...
    def putmessage(self, message):
        ...
        while len(s) > 0:
            try:
                # print >> sys.__stderr__, "putmessage select", 
                r, w, x = select.select([], [self.sock], [])
                # print >> sys.__stderr__, "end putmessage select", 
    ...
    def pollpacket(self, wait):
        ...
        if len(self.buffer) < self.bufneed:
            stackless.schedule()
            # print >> sys.__stderr__, "pollpacket select", 
            r, w, x = select.select([self.sock.fileno()], [], [], wait)
            # print >> sys.__stderr__, "end pollpacket select", 
            if len(r) == 0:
                return None
            try:
...
class RPCClient(SocketIO):
    ...
    def __init__(self, address, family=socket.AF_INET, type=socket.SOCK_STREAM):
        ...
        self.listening_sock.setblocking(0)
