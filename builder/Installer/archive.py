...
class Archive:
    ...
    def checkmagic(self):
        ...
        magic = self.lib.read(len(self.MAGIC))
        if  magic != self.MAGIC:
            raise RuntimeError, "%s is not a valid %s archive file %s != %s" \
              % (self.path, self.__class__.__name__, repr(magic), repr(self.MAGIC))
    ...
    def getmod(self, nm, newmod=imp.new_module):
        ...
        if ispkg:
            if _environ.has_key('_MEIPASS2'):
                ...
                localpath = iu._os_path_dirname(self.path) + '/support'
