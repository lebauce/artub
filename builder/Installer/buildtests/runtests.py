...
def clean():
    distdirs = glob.glob(os.path.join(here, 'dist*'))
    for dir in distdirs:
        try:
            empty(dir)
            os.rmdir(dir)
            ...
            print e
    builddirs = glob.glob(os.path.join(here, 'build*'))
...
def runtests():
    ...
    if sys.platform[:3] != 'win':
        print
        print "testing symbolic links"
        print
        os.system('%s ../Build.py symlink1.spec' % PYTHON)
        if os.path.samefile('buildsymlink1test/support/symlink1.foo',
            'buildsymlink1test/support/symlink1.spec'):
            print "passed"
        else:
            print "FAILED"
