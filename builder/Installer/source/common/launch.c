...
int loadPython()
{
    ...
    /* Determine the path */
    sprintf(dllpath, "%ssupport\\python%02d.dll", f_homepathraw, ntohl(f_cookie.pyvers));
...
int extract2fs(TOC *ptoc)
{
    ...
    if (!f_workpath) {
        ...
        if (!f_temppath[0]) {
            FATALERROR("Could not create temp directory\n");
            return -1;
        }
#ifdef WIN32
...
#ifndef WIN32
        else if (ptoc->typcd == 'l') {
            /* ptoc->name is src:dest */
            pos  = strchr(ptoc->name, ':');
            strcpy(dest, f_workpath);
            strncpy(src, ptoc->name, pos-ptoc->name); 
            src[pos-ptoc->name] = '\0';
            strcat(dest, pos+1);
