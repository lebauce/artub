...
def build(spec):
    ...
    if SPECPATH == '':
        SPECPATH = os.getcwd()
    SPECPATH = os.path.normpath(SPECPATH)
    ...
    def assemble(self):
        ...
        if oldstuff != (self.inputs, self.pathex, self.hookspath, self.excludes, scripts, pure, binaries):
            ...
            for ln in analyzer.getwarnings():
                ...
                wf.write(ln+'\n')
            wf.close()
            print "%sWarnings written to %s" % (["", "ERRORS and "][haserrors], WARNFILE)
            return 1
...
class ELFEXE(Target):
    ...
    def __init__(self, *args, **kws):
        ...
        if not self.name[:len(SPECPATH)] == SPECPATH:
...
class COLLECT(Target):
    def __init__(self, *args, **kws):
        ...
        for i in range(len(self.toc)):
            inm, fnm, typ = self.toc[i]
            if typ == 'EXTENSION':
                inm = fixextension(inm, fnm)
            if typ in ('EXTENSION', 'BINARY', 'LINK'):
                if not inm[:7] == 'support':
                    inm = os.path.join('support', inm)
                self.toc[i] = (inm, fnm, typ)
        self.__postinit__()
    def check_guts(self, last_build):
        outnm = os.path.basename(self.out)
        try:
        ...
        for inm, fnm, typ in self.toc:
            test = os.path.join(self.name, inm)
    ...
    def assemble(self):
        ...
        for inm, fnm, typ in self.toc:
            ...
            if typ == 'LINK':
                if iswin:
                    print "Link", os.path.join(todir, fnm), '->', tofnm
                else:
                    if os.path.exists(tofnm):
                        if os.readlink(tofnm) != fnm:
                            os.remove(tofnm)
                            os.symlink(fnm, tofnm)
                    else:
                        os.symlink(fnm, tofnm)
            else:
...
import imp
if not iswin:
    for suffix, junk, typ in imp.get_suffixes():
        if typ == imp.C_EXTENSION:
            C_EXTENSION_SUFFIX = suffix
            break
def fixextension(nm, pth):
    if iswin:
        binext = os.path.splitext(pth)[1]
    else:
        binext = C_EXTENSION_SUFFIX
    if not os.path.splitext(nm)[1] == binext:
        return nm+binext
    return nm
...
def TkPKG():
    return PKG(TkTree(), name=os.path.join(BUILDPATH, 'tk.pkg'))
