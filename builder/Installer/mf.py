    ...
    def getmod(self, nm, getsuffixes=imp.get_suffixes, loadco=marshal.loads):
        ...
        for pth, ispkg, pkgpth in possibles:
            for ext, mode, typ in getsuffixes():
                ...
                try:
                    ...
                    if iswin:
                        if ispkg: 
                            if nm not in dircache.listdir(self.path):
                                continue
                        else:
                            if nm+ext not in dircache.listdir(self.path):
                                continue
                    if typ == imp.C_EXTENSION:
                        if not iswin:
                            while os.path.islink(attempt):
                                link = os.readlink(attempt)
                                if os.path.isabs(link):
                                    attempt = link
                                else:
                                    attempt = os.path.join(os.path.dirname(attempt), link)
                                    attempt = os.path.normpath(attempt)
        ...
        while 1:
            if pyc is None or py and pyc[1][8] < py[1][8]:
                try:
                    co = compile(open(py[0], TEXTMODE).read()+'\n', py[0], 'exec')
                    if __debug__:
                        pth = py[0] + 'c'
                    else:
                        pth = py[0] + 'o'
    ...
    def getmod(self, nm):
        ...
        if stuff:
            ...
            if typ == imp.C_EXTENSION:
                ...
                try:
                    co = compile(open(fnm, TEXTMODE).read()+'\n', fnm, 'exec')
                except SyntaxError, e:
        ...
        if i<len(nmparts):
            if ctx:
                ...
                if not self.ispackage(ctx):
                    return nms
            elevel = 'W'
            if imptyp == 0 and importernm == '__main__' and string.find(fqname, '.') == -1:
                elevel = 'E'
            self.warnings["%s: no module named %s (%s import by %s)" % (elevel, fqname, imptyps[imptyp], importernm or "__main__")] = 1
